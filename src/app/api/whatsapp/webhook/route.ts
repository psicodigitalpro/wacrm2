...
async function processMessage(
  ...
  // Reactions short-circuit before parseMessageContent (skips the
  // media-URL fetch) — build the normalized message directly from the
  // raw reaction fields; processInboundMessage handles the
  // find-or-create + upsert/delete itself.
  if (message.type === 'reaction') {
    ...
    const normalized: NormalizedInboundMessage = {
      ...
      reaction: { targetProviderMessageId: message.reaction.message_id, emoji: message.reaction.emoji },
    }
    await processInboundMessage(normalized)
    return
  }

  // Parse message content based on type — Meta-specific (verifies media
  // against Meta's API using the account's access token).
  const { contentText, mediaUrl, mediaType, interactiveReplyId } =
    await parseMessageContent(message, accessToken)
  // `mediaType` is intentionally unused beyond parseMessageContent — the
  // schema has no media_type column; the MIME type only drives the
  // proxy URL construction inside parseMessageContent itself.
  void mediaType
  ...
  // allowed value — processInboundMessage also guards this, but resolve
  // the sticker→image mapping here since it's Meta-specific vocabulary.
